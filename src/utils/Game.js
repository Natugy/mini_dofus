...
export class Game {
	...
	initialiserGame() {
		...
		this.monstresEnJeu.push(
			Object.assign(new Monstre(), GameData.listeMonstres[0])
		);
		this.resetCarte();
	...
	resetCarte() {
		...
		this.map[this.joueur.y][this.joueur.x].addEventListener('click', () =>
			this.activerMouvement()
		);
		this.monstresEnJeu.forEach(monstre => {
			this.map[monstre.y][monstre.x].estMonstre(monstre);
			this.map[monstre.y][monstre.x].addEventListener('click', () =>
				this.afficherInfosEnnemi(monstre)
			);
		});
	}
	...
	creationMonstre() {
		...
		const newMonstre = Object.assign(
			new Monstre(),
			GameData.listeMonstres[
				Math.floor(
					Math.random() *
						Math.min(
							Math.floor(this.tour / 3) + 1,
							GameData.listeMonstres.length
						)
				)
			]
		);
		...
		LogMessage.logMessageInfo(
			`${newMonstre.nom} apparaît en (${newMonstre.x}, ${newMonstre.y})!`
		);
		this.resetCarte();
		this.mettreAJourInterface();
	}
	...
	afficherInfosEnnemi(ennemi) {
		...
		LogMessage.logMessageInfo(
			`Nom: ${ennemi.nom} PV: ${ennemi.pv} Attaque: ${ennemi.attaque} Defense: ${ennemi.defense} PA : ${ennemi.pa}`
		);
	}
			...
			ligne.forEach(cellule => {
				if (this.joueur.estAPorteeDeplacement(cellule) && cellule.estLibre()) {
					...
					cellule.addEventListener('click', () =>
						this.deplacerJoueur(cellule.x, cellule.y)
					);
				}
			});
		});
	...
	deplacerJoueur(x, y) {
		...
		if (this.map[y][x].estLibre() === false) {
			LogMessage.logMessageErreur(
				"Vous ne pouvez pas vous déplacer sur la case de l'ennemi."
			);
			...
			for (let j = 0; j < this.mapSize; j++) {
				if (i === this.joueur.x || j === this.joueur.y) {
					if (sort.estAPortee({ x: i, y: j })) {
						...
						this.map[j][i].addEventListener('click', () =>
							this.lancerSort(sort, i, j)
						);
					}
				}
			}
		...
		for (let y = 0; y < this.mapSize; y++) {
			for (let x = 0; x < this.mapSize; x++) {
				if (sort.estAPortee({ x, y })) {
					...
					this.map[y][x].addEventListener('click', () =>
						this.lancerSort(sort, x, y)
					);
				}
			}
		}
	...
	viseeSortDiagonale(sort) {
		...
		for (let i = 1; i < this.mapSize; i++) {
			if (
				this.checkCoordonnees(joueur.x + i, joueur.y + i) &&
				sort.estAPortee({ x: joueur.x + i, y: joueur.y + i })
			) {
				this.map[joueur.y + i][joueur.x + i].afficherPorteeSort();
				this.map[joueur.y + i][joueur.x + i].addEventListener('click', () =>
					this.lancerSort(sort, joueur.x + i, joueur.y + i)
				);
			}
			if (
				this.checkCoordonnees(joueur.x - i, joueur.y + i) &&
