...
export class Sort {
	...
	estAPortee(cible) {
		const distance =
			Math.abs(this.lanceur.x - cible.x) + Math.abs(this.lanceur.y - cible.y);
	...
	lancerSort(cible) {
		if (this.coutPA <= this.lanceur.pa && this.estAPortee(cible)) {
			...
			const degats = Math.max(
				this.degats + this.lanceur.attaque - cible.defense,
				0
			);
			...
			LogMessage.logMessageCombat(
				`${this.lanceur.nom} lance ${this.nom} sur ${cible.nom} et inflige ${degats} points de dégâts.`
			);
		} else {
